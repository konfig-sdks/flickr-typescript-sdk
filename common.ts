...
import { readableStreamToString, FlickrError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new FlickrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new FlickrError(e, e.response?.data)
                }
                if (innerError instanceof FlickrError) {
                    // Got "FlickrError" from the above try block
                    throw innerError;
                }
